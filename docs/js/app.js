...
function generateArtifacts() {
  $('#artifacts').empty();
  $.each(artifacts.data, (k, v) => {
    if (isNaN(v.level)) {
      v.level = 0;
    }
    let div = `<div class="artifact${v.active == 1 ? '' : ' ignore'}" id="${k}div"><input type="checkbox" id="${k}active"${v.active == 1 ? ' checked="checked"' : ''} onchange="updateActive('${k}');" tabindex="-1" /><label><input id="${k}" value="${v.level}" type="tel" onchange="updateArtifact('${k}')" />${v.name}</label><br /><span id="${k}effect">`;
...
function generateUpgrades() {
  ...
  let forceBOS = parseInt($('#forcebos').val());
  let relics = new Decimal(`${$('#relics').val() == '' ? 0 : $('#relics').val()}.${$('#relics_decimal').val() == '' ? 0 : $('#relics_decimal').val()}`);
  ...
  upgrades = {};
  let temp_artifacts = $.extend(true, {}, artifacts);
  var litmus = false;
  $.each(temp_artifacts.data, (k, v) => {
  ...
  while (forceBOS > 0 && $('#ocd').prop('checked') == false) {
    if ($('#bos_type').val() == 'level') {
      ...
      const bos_pct = forceBOS / 100;
      let bos_relics = relics * bos_pct;
      while (true) {
  ...
  if ($('#ocd').prop('checked')) {
    $.each(artifacts.data, (k, v) => {
      if (k in upgrades) {
        const x = Math.floor(temp_artifacts.data[k].level / 100) * 100;
        ...
        } else if (artifacts.data[k].max == -1) {
          delete upgrades[k];
        }
      }
    });
...
function oldEff(data, k, v) {
  const current_ad = v.level * v.ad;
  const current_effect = 1 + v.effect * Math.pow(v.level, Math.pow((1 + (v.cexpo - 1) * Math.min(v.grate * v.level, v.gmax)), v.gexpo));
  data.data[k].current_ad = current_ad;
  data.data[k].current_effect = current_effect;
  if (v.max == -1 || v.max > v.level) {
    const cost = Math.pow(v.level + 1, v.cexpo) * v.ccoef;
    data.data[k].cost = cost;
    data.data[k].displayCost = displayTruncated(cost);
    const next_effect = 1 + v.effect * Math.pow(v.level + 1, Math.pow((1 + (v.cexpo - 1) * Math.min(v.grate * (v.level + 1), v.gmax)), v.gexpo));
    const effect_diff = next_effect / current_effect;
    const effect_eff = Math.pow(effect_diff, v.rating);
    const ad_change = (((v.level + 1) * v.ad) - current_ad);
    const ad_eff = 1 + (ad_change / data.totalAD);
    const eff = Math.abs(((effect_eff * ad_eff) - 1) / cost);
...
function newEff(data, k, v, avglvl, cost) {
  ...
  let i = 1;
  const j = (v.max == -1 || v.max > avglvl ? avglvl : v.max);
  while (i <= j) {
  ...
  const effect_eff = Math.pow(next_effect, v.rating);
  const ad_eff = 1 + ((avglvl * v.ad) / data.totalAD);
  const eff = Math.abs(((effect_eff * ad_eff) - 1) / cost);
  data.data[k].efficiency = eff;
  return (data);
}
...
function calculate(data, k, regenerate, pinch) {
  const next_artifact = countArtifacts(artifacts.data) + 1;
  const next_artifact_cost = artifact_costs[next_artifact];
  const average_level = determineAverage(artifacts.data);
  const v = data.data[k];
  data.data[k].efficiency = -1;
  data.data[k].cost = '';
  data.data[k].displayCost = '';
  if (v.level > 0 && v.active == 1) {
    const prior_ad = v.current_ad;
    data = oldEff(data, k, v);
    const new_ad = data.data[k].current_ad;
  } else if (v.level == 0 && next_artifact_cost != -1 && v.active == 1 && pinch === true) {
  ...
  winner_value = 0;
  $.each(data.data, (k, v) => {
    if (v.efficiency != -1 && v.efficiency > winner_value) {
      if (v.level > 0 && v.active == 1) {
        winner_e = k;
...
function calculateAll(data, regenerate) {
  ...
  $.each(data.data, (k, v) => {
    ...
    if (v.level > 0 && v.active == 1) {
      data = oldEff(data, k, v);
      if (data.data[k].efficiency != -1 && data.data[k].efficiency > winner_value) {
        winner_e = k;
        temp_winner_n = '';
...
if (storageAvailable('localStorage')) {
  const localArtifacts = JSON.parse(window.localStorage.getItem('artifacts'));
  if (localArtifacts != null && typeof localArtifacts.data === 'undefined') {
    localArtifacts.data = jQuery.extend(true, {}, localArtifacts);
  }
  if (localArtifacts != null && typeof localArtifacts.data !== 'undefined') {
    $.each(localArtifacts.data, (k, v) => {
...
const origWeights = jQuery.extend(true, {}, artifacts.data);
generateArtifacts();
